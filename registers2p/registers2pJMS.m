...
function varargout = registers2pJMS(varargin)
% REGISTERS2PJMS MATLAB code for registers2pJMS.fig
%      REGISTERS2PJMS, by itself, creates a new REGISTERS2PJMS or raises the existing
%      singleton*.
%
%      H = REGISTERS2PJMS returns the handle to a new REGISTERS2PJMS or the handle to
%      the existing singleton*.
%
%      REGISTERS2PJMS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in REGISTERS2PJMS.M with the given input arguments.
%
%      REGISTERS2PJMS('Property','Value',...) creates a new REGISTERS2PJMS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before registers2pJMS_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to registers2pJMS_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help registers2pJMS

% Last Modified by GUIDE v2.5 02-Mar-2018 15:09:26

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;  
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @registers2pJMS_OpeningFcn, ...
                   'gui_OutputFcn',  @registers2pJMS_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before registers2pJMS is made visible.
function registers2pJMS_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to registers2pJMS (see VARARGIN)

% Choose default command line output for registers2pJMS
...
%     handles.files(i).image{1} = zeros(512,512,handles.files(i).num_masks);
    handles.files(i).image{1} = zeros(219,219,handles.files(i).num_masks);
...
% UIWAIT makes registers2pJMS wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = registers2pJMS_OutputFcn(hObject, eventdata, handles) 
