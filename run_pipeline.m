...
opath = sprintf('%sregops_%s_%s.mat', ops0.ResultsSavePath, ops0.mouse_name, ops0.date);
processed = 1;
if exist(opath, 'file')
    load(opath);
    for j = 1:numel(ops1)       
...
%     ops.ThScaling = 0.5;
    ...
    if ops.getROIs
        ...
        cycleLength = 32;
        if strcmpi(db.stimType,'epOri')
            [TCs, TCfits, paras] = getTC_Suite2P_epORI(stat,Fcell,FcellNeu,cycleLength);

        elseif strcmpi(db.stimType,'epOri')
            [TCs, TCfits, paras] = getTC_Suite2P_epORI(stat,Fcell,FcellNeu,cycleLength);
            
            for i = 1:length(stat);
                stat(i).TC = TCs(i,:);
                stat(i).TCfit = TCfits(i,:);
                stat(i).paras = paras(i,:);
            end
        end        
